function addFontSizeToStyle(style, e) {
  let fontSize =
    e.bounds && e.bounds.width > 240 && e.bounds.height > 120 ? 16 : 11;
...
function isExternElement(e) {
  ...
  for (let i = 0; i < parents.length; i++) {
    ...
    if (
      parents[i].type === "folder" &&
      parents[i].name &&
      parents[i].name.toLowerCase() === "extern"
    ) {
      return true;
    }
  }
...
function createConnector(
  ...
  }" edge="1" parent="${parent}" source="${e.source.id}" target="${
    e.target.id
  }">
...
function createConnectorWithTopic(
  ...
  let topicElem = `   <object id="${topicId}" label="${topicLabel}">
        <mxCell style="${topicStyle}" vertex="1" parent="${parent}">
            <mxGeometry x="${topicX - 30}" y="${
    topicY - 30
  }" width="60 pt" height="35 pt" as="geometry" />
        </mxCell>
    </object>
`;
...
function c4ObjLabel(c4Name, c4Type, c4Description, fontColor) {
  ...
  let showTypeAndDesc = wellKnownC4Types
    .map((t) => t.toLowerCase())
    .includes((c4Type || "").toLowerCase());
...
const colorFactory = () =>
  Java.type("com.archimatetool.editor.ui.ColorFactory");
